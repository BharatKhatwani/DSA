package LIstInterferfaceExamples;
import com.sun.security.jgss.GSSUtil;

import java.util . *;
public class c2_QueueInterfaceExamples {
    public static void main(String[] args) {
//QueueExamples();
      PriorityQueueExample();
    }
    static void QueueExamples (){
        LinkedList <Integer> q = new LinkedList<>();
        q.offer(1);    //  offer :--- add similar
        q.offer(2);
        q.offer(3);
        q.offer(4);
        q.offer(5);
        System.out.println(q.peek());  // 1
        System.out.println(q.poll());  // 1
        System.out.println(q.peek());   //2
        System.out.println(q.isEmpty());     // false
        System.out.println(q.size());   // 2
        System.out.println(q);
    }
    static void PriorityQueueExample(){
      //  PriorityQueue <Integer> pq= new PriorityQueue<>();   // min pq
        PriorityQueue <Integer> pq= new PriorityQueue<>(Comparator.reverseOrder()); // max pq
        pq.add(10);
        pq.add(5);
        pq.add(7);
        System.out.println(pq.peek());  // 5 -> the smallest at has the highest queue
        System.out.println(pq); // Guarantee ;- topmost priority elt will be provcasses
        System.out.println(pq.poll());  // 5
        System.out.println(pq);
        System.out.println(pq.peek());  // 7

    }
    public class QueueInterfaceExamples{
    static   void DequeExamples() {
        Deque<Integer> dq = new ArrayDeque<>();
        dq.addFirst(1);
        dq.addFirst(2);
        dq.addLast(3);
        dq.addLast(4);
        System.out.println(dq);
        System.out.println(dq.pollFirst());
        System.out.println(dq.pollLast());  // poll :-  remove
        System.out.println(dq);
    }
    }

}
